...
def calculate_angle_between_vectors(v1, v2):
    ...
    cos_theta = np.dot(v1, v2) / (np.linalg.norm(v1) * np.linalg.norm(v2)) # 求cos
    cos_theta = np.clip(cos_theta, -1.0, 1.0)                              # cos限制在[-1, 1]
    angle_radians = np.arccos(cos_theta)                                   # 取arccos
...
class HandRetarget:
    ...
    def _get_point_angle(self, finger_frames, origin, point1, point2):
        vector1 = finger_frames[point1, :3, 3] - finger_frames[origin, :3, 3]  # 向量V65
        vector2 = finger_frames[point2, :3, 3] - finger_frames[origin, :3, 3]  # 向量V69
        
        angle = calculate_angle_between_vectors(vector1, vector2) / np.pi * 180 # 计算弧度并转成角度
        return angle

    def _solve_four_fingers(self, finger_frames):
        # 顺序是 (little, ring, middle, index)
        four_angles = np.zeros(4)   # 用来存储除大拇指的关节角度
        for i in range(4):
            # 6 to 9, 6 to 5 is index finger
            # plus 5 per finger
            angle = self._get_point_angle(finger_frames, 6+5*i, 5+5*i, 9+5*i)   # 6 5 9
            four_angles[3-i] = angle  # 倒着排

        # 这里两个值应该是人手打开和握拳的角度，映射到0到1000之间
        # 机械手的角度在19到176.7之间
        four_angles = np.clip(four_angles, *self.four_fingers_limits)  # 限制道40-170
        # 缩放到0-1000
    ...
    def _solve_thumb(self, finger_frames, pinch_distance):
        ...
        bending_angle = np.clip(bending_angle, *self.thumb_bending_limits)  # 15-30度
        bending_angle = (bending_angle - self.thumb_bending_limits[0]) / (self.thumb_bending_limits[1] - self.thumb_bending_limits[0]) * 1000

        # rotation
        # 人手角度在xx和xx之间，映射到0到1000，
        # 机械手值 90deg 到 165deg
        rotation_angle = np.clip(rotation_angle, *self.thumb_rotation_limits)  # 80.0 - 150.0
    ...
    def solve_fingers_angles(self, r):
        ...
        # 初始判断需不需要保存cache, 如果 left有效值是空的就保存当前的
        ...
        left_thumb_angles = self._solve_thumb(finger_frames, pinch_distance)
