...
class ArmRetarget:
    def __init__(self, config_file="open_loong.yaml"):
        ...
        self.last_optimized_q_10d = np.array(self.config['init_q']) * np.pi / 180
    ...
    def _joint_velocity_cost(self, q0_10d, q_10d):
        n, s, c, r = 1, 0, 0.2, 5.0
        xll = q0_10d[0:5] - q_10d[0:5]  # 前5个joint相减
        xll[0:3] = xll[0:3] * 2         # 前3个joint取平方
        xl = np.linalg.norm(xll)        # 求向量的模
    ...
    def _objective_function(self, q_10d):
        ...
        vel_cost = self._joint_velocity_cost(self.last_optimized_q_10d, q_10d)  # 就是joint误差
    ...
    def _solve_uparm_angles(self, q0, target_pos, target_quat):
        ...
        result = opt.minimize(
            self._objective_function,  # return 60 * pos_cost + 6 * ori_cost + 2 * vel_cost
            q0_10d,
            method="SLSQP",
            tol=1e-3,
            bounds=self.bounds,
    ...
    def solve_arm_angles(self, q0, r):
        ...
        if is_left_missing or is_right_missing: # 如果没有检测到手，r = 上一次
            r = self.last_valid_r
        else:                      #如果检测到手了, 上一次的值就更新
            self.last_valid_r = r
        ...
        # left_fingers[25]手部检测的腕关节坐标 左乘 T_wrist
        left_forearmWrist = left_wrist @ r["left_fingers"][-2]
        right_forearmWrist = right_wrist @ r["right_fingers"][-2]

        # 计算腕部的2个q, 最后2个qpos
        q_2d_l = self._solve_wrist_angles(left_wrist, left_forearmWrist, chirality="left")
        q_2d_r = self._solve_wrist_angles(right_wrist, right_forearmWrist, chirality="right")
